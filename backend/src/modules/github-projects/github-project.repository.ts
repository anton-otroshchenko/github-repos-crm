...
class GithubProjectRepository {
  ...
  public async findByUserAndRepo(userId: number, owner: string, repoName: string): Promise<null | any> {
    return this.githubProjectModel
        .query()
        .findOne({
          user_id: userId,
  ...
  public async update(id: number, data: Partial<GithubProjectModel>): Promise<any> {
    return this.githubProjectModel
        .query()
  ...
  public async bulkCreate(projects: Partial<GithubProjectModel>[]): Promise<any[]> {
